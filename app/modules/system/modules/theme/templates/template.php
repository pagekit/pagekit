...
<html lang="<?= str_replace('_', '-', $app['translator']->getLocale()) ?>">
    <head>
        ...
        <link href="<?= $view->url()->getStatic('app/modules/system/modules/theme/favicon.ico') ?>" rel="shortcut icon" type="image/x-icon">
        <link href="<?= $view->url()->getStatic('app/modules/system/modules/theme/apple_touch_icon.png') ?>" rel="apple-touch-icon-precomposed">
        <link href="//fonts.googleapis.com/css?family=Open+Sans:300,400,600&amp;subset=<?= $subset ?>" rel="stylesheet">
        <?= $view->section()->render('head') ?>
        <?php $view->style('theme', 'app/modules/system/modules/theme/css/theme.css') ?>
        <?php $view->script('theme', 'app/modules/system/modules/theme/js/theme.js', ['uikit', 'uikit-notify', 'uikit-sticky', 'uikit-sortable']) ?>
    </head>
    <body>
