jQuery(function ($) {
    ...
    UIkit.modal.confirm = function(content, onconfirm, options) {
        ...
        var modal = UIkit.modal.dialog(([
            options.title ? '<div class="uk-modal-header"><h2>'+options.title+'</h2></div>':'',
            '<div class="uk-margin uk-modal-content">'+String(content)+'</div>',
            '<div class="uk-modal-footer uk-text-right"><button class="uk-button uk-button-link js-modal-confirm">'+options.labels.Ok+'</button> <button class="uk-button uk-button-link uk-modal-close">'+options.labels.Cancel+'</button></div>'
        ]).join(""), options);
    ...
    UIkit.modal.prompt = function(text, value, onsubmit, options) {
        ...
        var modal = UIkit.modal.dialog(([
            ...
            '<div class="uk-modal-footer uk-text-right"><button class="uk-button uk-button-link js-modal-ok">'+options.labels.Ok+'</button> <button class="uk-button uk-button-link uk-modal-close">'+options.labels.Cancel+'</button></div>'
        ]).join(""), options),
