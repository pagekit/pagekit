...
<div class="uk-grid" data-uk-grid-margin data-uk-grid-match>
    <div class="pk-sidebar uk-width-medium-1-4">
        <form id="js-access-level" class="uk-form" method="post">
            ...
            <ul class="pk-sortable" data-uk-sortable="{ maxDepth: 1, prefix: 'pk' }" data-update-url="@url.to('@system/accesslevel/priorityupdate')">
                ...
                <li data-id="@lev.id">
                    <div class="pk-sortable-item uk-visible-hover@( lev == level ? ' pk-active' )">
                        ...
                        <ol class="uk-subnav pk-subnav-icon uk-hidden">
                            <li><a href="#" data-edit="@url.to('@system/accesslevel/save', ['id' => lev.id])" data-name="@lev.name" title="@trans('Edit')"><i class="uk-icon-pencil"></i></a></li>
                            <li><a href="#" data-action="@url.to('@system/accesslevel/delete', ['id' => lev.id])" data-confirm="@trans('Are you sure?')" title="@trans('Delete')"><i class="uk-icon-minus-circle"></i></a></li>
                        </ol>
                        @endif
                        <a href="@url.to('@system/accesslevel/index', ['id' => lev.id])">@lev.name</a>
                    </div>
                </li>
                @endforeach
            </ul>
            <hr>
            <a class="uk-button" href="#" data-edit="@url.to('@system/accesslevel/save', ['id' => 0])">@trans('Add Access Level')</a>
            @token()

        </form>
    </div>
    <div class="pk-content uk-width-medium-3-4">
        <form id="js-access-level-roles" class="uk-form" action="@url.to('@system/accesslevel/save', ['id' => level.id])" method="post">
