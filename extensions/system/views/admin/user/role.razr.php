...
<div class="uk-grid" data-uk-grid-margin data-uk-grid-match>
    <div class="pk-sidebar uk-width-medium-1-4">
        <form id="js-role" class="uk-form" method="post">
            ...
            <ul class="pk-sortable" data-uk-sortable="{ maxDepth: 1, prefix: 'pk' }" data-update-url="@url.to('@system/role/priorityupdate')">
                @foreach (roles as r)
                <li data-id="@r.id">
                    <div class="pk-sortable-item uk-visible-hover@( r == role ? ' pk-active')">
                        ...
                        <ol class="uk-subnav pk-subnav-icon uk-hidden">
                            <li><a href="#" data-edit="@url.to('@system/role/save', ['id' => r.id])" data-name="@r.name" title="@trans('Edit')"><i class="uk-icon-pencil"></i></a></li>
                            <li><a href="#" data-action="@url.to('@system/role/delete', ['id' => r.id])" data-confirm="@trans('Are you sure?')" title="@trans('Delete')"><i class="uk-icon-minus-circle"></i></a></li>
                        </ol>
                        @endif
                        <a href="@url.to('@system/role/index', ['id' => r.id])">@r.name</a>
                    </div>
                </li>
                @endforeach
            </ul>
            <hr>
            <a class="uk-button" href="#" data-edit="@url.to('@system/role/save', ['id' => 0])">@trans('Add Role')</a>
            @token()

        </form>
    </div>
    <div class="pk-content uk-width-medium-3-4">
        <form id="js-role-permissions" class="uk-form" action="@url.to('@system/role/save', ['id' => role.id])" method="post">
